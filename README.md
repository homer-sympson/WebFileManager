...
**Attach к уже работающему контейнеру** (Debug → Attach to Process, Connection type «Docker (Linux
Container)», target `filemanager-app-1`): в списке будет `dotnet` с **PID 1** от имени `root` — это
приложение, запущенное entrypoint'ом (`exec dotnet …`), поэтому у него первый PID. Привязка теперь
проходит (для неё нужны `cap_add: SYS_PTRACE` и dumpable-флаг — см. troubleshooting выше), но
учитывайте:

* образ собран внутри контейнера, поэтому пути исходников в PDB — сборочные (`/src/src/...`); VS может
  не найти файлы автоматически и предложит указать их вручную;
* это **Release**-сборка с оптимизациями, поэтому часть точек останова сместится или не привяжется
  (полые кружки с подсказкой). Символы в образе есть — `.pdb` публикуются рядом с `dll`;
* полноценная отладка (точки останова, шаг с заходом, горячая перезагрузка) — это **F5 на профиле
  «Docker Compose»**: VS монтирует Debug-сборку в `/app` и запускает её под `vsdbg` из тех же путей,
  что и на хосте, поэтому исходники сопоставляются сразу.
